<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        @if ($user->avatarUrl())
            <div class="mb-4">
                <img src="{{ $user->avatarUrl() }}" alt="{{ $user->name }}" class="w-21 h-20 rounded-full object-cover">
            </div>
        @endif
        ...
        <div class="mb-4">
             ...
             <textarea name="bio" id="bio" rows="6" class="w-full border border-gray-300 rounded-md p-2" required>{{ old('bio', $user->bio) }}</textarea>
            @error('bio')
             <span class="text-red-500 text-sm">{{ $message }}</span>
            @enderror
        </div>
